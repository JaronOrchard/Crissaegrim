...
public class Tile2LHI extends Tile {
	
	public Tile2LHI() {
		super(Textures.NONE, Textures.TILE_2LHI, Textures.NONE);
	}
	
	public Tile2LHI(int fgTexture, int mgTexture, int bgTexture) {
	...
	protected List<Line> getTileBoundaryLines(int xPos, int yPos) {
		return Arrays.asList(
				...
				new Line(new Coordinate(xPos + 1, yPos + 1), new Coordinate(xPos + 1, yPos)),
				new Line(new Coordinate(xPos + 1, yPos), new Coordinate(xPos, yPos + 0.5)),
				new Line(new Coordinate(xPos, yPos + 0.5), new Coordinate(xPos, yPos + 1)));
	}
