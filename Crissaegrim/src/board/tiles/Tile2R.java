...
public class Tile2R extends Tile {
	
	public Tile2R() {
		super(Textures.NONE, Textures.TILE_2R, Textures.NONE);
	}
	
	public Tile2R(int fgTexture, int mgTexture, int bgTexture) {
	...
	public boolean playerBodyCollides(int xPos, int yPos, Player player, Coordinate startingPosition, Coordinate endingPosition) {
		...
		tileBoundaryLines.add(new Line(new Coordinate(xPos + 1, yPos), new Coordinate(xPos, yPos + 0.5)));
	...
	protected Coordinate raisePositionToAboveTile(int xPos, int yPos, Coordinate position) {
		return new Coordinate(position.getX(), yPos + Math.max(Math.min(0.5 - ((position.getX() - (double)xPos) / 2), 0.5), 0) + PlayerMovementHelper.getTileCollisionPadding());
	}
